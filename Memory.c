...
unsigned char		PagedMem [ 16 ][ 16384 ];
...
void
WriteByte ( unsigned int addr, unsigned char val )
{
	/*
	...
	{
		Mem [ addr ] = val;
		if ( addr >= StartOfScreenMemory && addr < EndOfScreenMemory )
			ScreenChanged++;
		return;
	}
...
void
LoadPagedRom ( char *romname, unsigned char socket )
{
	int				fd;
