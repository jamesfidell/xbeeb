...
ReadSheila ( int addr )
{
	...
	if ( addr < 0xfea0 )
		return ReadFloppyController ( addr - 0xfe80 );
	...
	fprintf ( stderr, "Illegal address in ReadSheila (%x)\n", addr );
	exit ( 1 );
}


void
WriteSheila ( int addr, unsigned char val )
{
	/*
	 * 0xfe00 - 0xfe07 : 6845 CRTC
	 */
	...
	if ( addr < 0xfea0 )
	{
		WriteFloppyController ( addr - 0xfe80, val );
		return;
	}
