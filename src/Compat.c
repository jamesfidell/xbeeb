/*
 *
 * $Id: Compat.c,v 1.4 2002/01/15 15:46:43 james Exp $
 *
 * Copyright (C) James Fidell 1994-2002.
 *
 * Permission to use, copy, modify and distribute this software
 * and its documentation for any purpose is hereby granted without fee,
 * provided that the above copyright notice appear in all copies and
 * that both that copyright notice and this permission notice appear in
 * supporting documentation, and that the name of the copyright holder
 * not be used in advertising or publicity pertaining to distribution
 * of the software without specific, written prior permission. The
 * copyright holder makes no representations about the suitability of
 * this software for any purpose. It is provided "as is" without express
 * or implied warranty.
 *
 * THE COPYRIGHT HOLDER DISCLAIMS ALL WARRANTIES WITH REGARD TO THIS
 * SOFTWARE, INCLUDING ALL IMPLIED WARRANTIES OF MERCHANTABILITY AND
 * FITNESS, IN NO EVENT SHALL THE COPYRIGHT HOLDER BE LIABLE FOR ANY
 * SPECIAL, INDIRECT OR CONSEQUENTIAL DAMAGES OR ANY DAMAGES WHATSOEVER
 * RESULTING FROM LOSS OF USE, DATA OR PROFITS, WHETHER IN AN ACTION OF
 * CONTRACT, NEGLIGENCE OR OTHER TORTIOUS ACTION, ARISING OUT OF OR IN
 * CONNECTION WITH THE USE OR PERFORMANCE OF THIS SOFTWARE.
 *
 */

/*
 * Modification History
 *
 * $Log: Compat.c,v $
 * Revision 1.4  2002/01/15 15:46:43  james
 * *** empty log message ***
 *
 * Revision 1.3  2000/08/16 17:58:26  james
 * Update copyright message
 *
 * Revision 1.2  1996/10/13 17:12:47  james
 * Add local version of "strtoul" for those systems that don't have it.
 *
 * Revision 1.1  1996/10/12 16:02:10  james
 * Moved my own version of the strcasecmp function into Compat.c
 *
 *
 */


#include "Config.h"
#include "Compat.h"


#ifdef	NEED_STRCASECMP

int
strcasecmp ( char *s, char *t )
{
	char		m, n;

	/*
	 * Degenerative cases first
	 */

	if ( !s && !t )
		return 0;

	if ( !s )
		return 1;

	if ( !t )
		return -1;

	while ((( m = toupper ( *s++ )) == ( n = toupper ( *t++ ))) && m != '\0' );

	return m - n;
}

#endif	/* NEED_STRCASECMP */


#ifdef	NEED_STRTOUL

unsigned long
strtoul ( char *s, char **e, int base )
{
	/*
	 * Degenerative cases first
	 */

	if ( !s || ! *s )
	{
		*e = s;
		return 0;
	}

	while ( ! isspace ( s ))
		s++;

	if ( base == 16 || ( base == 0 && *s == '0' && * ( s + 1 ) == 'x' ))
	{
		/*
		 * FIX ME
		 *
		 * Hex conversion
		 */
	}

	if ( base == 8 || ( base == 0 && *s == '0' ))
	{
		/*
		 * FIX ME
		 *
		 * Octal conversion
		 */
	}

	if ( base == 0 || base == 10 )
	{
		/*
		 * FIX ME
		 *
		 * Decimal Conversion
		 */
	}

	/*
	 * FIX ME
	 *
	 * This should return an error...
	 */

	return 0;
}

#endif	/* NEED_STRTOUL */
